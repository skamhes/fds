...
LOGICAL, INTENT(IN), OPTIONAL :: QUICK_READ
LOGICAL                       :: INVERSE ! INVERTS THE SHAPE
...
NAMELIST /OBST/ ALLOW_VENT,BNDF_FACE,BNDF_OBST,BULK_DENSITY,CELL_SIZE,CELL_SIZE_FACTOR,COLOR,CTRL_ID,DEVC_ID,FYI,HEIGHT,ID,&
                INTERNAL_HEAT_SOURCE,INVERSE,LENGTH,MATL_ID,MATL_MASS_FRACTION,MULT_ID,N_LAYER_CELLS_MAX,ORIENTATION,OUTLINE,&
                OVERLAY,PERMIT_HOLE,RADIUS,RAMP_IHS,REMOVABLE,RGB,SHAPE,STRETCH_FACTOR,SURF_ID,SURF_ID_INTERIOR,SURF_ID6,SURF_IDS,&
         ...
         J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
            I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
               ...
               IF (SHAPE_TYPE>0) THEN
                  ...
                  END SELECT
                  IF ((.NOT.IS_INTERSECT) .AND. (.NOT.INVERSE)) THEN
                     N = N-1
                     N_OBST = N_OBST-1
                     CYCLE I_MULT_LOOP
                  ENDIF
                  IF ((IS_INTERSECT) .AND. (INVERSE)) THEN
                     N = N-1
                     N_OBST = N_OBST-1
                     CYCLE I_MULT_LOOP
                  ENDIF
...
USE MISC_FUNCTIONS, ONLY: PROCESS_MESH_NEIGHBORHOOD
USE COMPLEX_GEOMETRY, ONLY: INTERSECT_CONE_AABB,INTERSECT_CYLINDER_AABB,INTERSECT_SPHERE_AABB,INTERSECT_OBB_AABB,ROTATION_MATRIX
CHARACTER(LABEL_LENGTH) :: DEVC_ID,CTRL_ID,MULT_ID,SHAPE
CHARACTER(25) :: COLOR
INTEGER :: NM,N_HOLE,NN,NDO,N,I1,I2,J1,J2,K1,K2,RGB(3),N_HOLE_NEW,N_HOLE_O,II,JJ,KK,NNN,DEVC_INDEX_O,CTRL_INDEX_O,SHAPE_TYPE
REAL(EB) :: X1,X2,Y1,Y2,Z1,Z2,TRANSPARENCY,HEIGHT,LENGTH,ORIENTATION(3),RADIUS,THETA,WIDTH,XYZ(3),AABB(6),ROTMAT(3,3)
LOGICAL :: IS_INTERSECT
NAMELIST /HOLE/ COLOR,CTRL_ID,DEVC_ID,FYI,HEIGHT,ID,LENGTH,MULT_ID,ORIENTATION,RADIUS,RGB,SHAPE,THETA,TRANSPARENCY,&
                WIDTH,XB,XYZ
...
READ_HOLE_LOOP: DO N=1,N_HOLE_O
   ...
   THETA       = 0._EB
   SHAPE_TYPE  = -1
   ! SHAPE_AREA  = 0._EB ! Not sure if this will come into use
   ...
   ! Classify any special shapes

   IF (TRIM(SHAPE)/='null') THEN

      ! specify shape type
      SELECT CASE(TRIM(SHAPE))
         CASE('SPHERE');    SHAPE_TYPE = HOLE_SPHERE_TYPE
         CASE('CYLINDER');  SHAPE_TYPE = HOLE_CYLINDER_TYPE
         CASE('CONE');      SHAPE_TYPE = HOLE_CONE_TYPE
         CASE('BOX');       SHAPE_TYPE = HOLE_BOX_TYPE
      END SELECT
      
      ! detect input errors
      IF ((SHAPE_TYPE==HOLE_SPHERE_TYPE .OR. SHAPE_TYPE==HOLE_CYLINDER_TYPE .OR. SHAPE_TYPE==HOLE_CONE_TYPE) &
         .AND. RADIUS<0._EB) THEN
         WRITE(MESSAGE,'(A,I0,A)')  'ERROR(602): HOLE ',N,' SHAPE requires RADIUS.'
         CALL SHUTDOWN(MESSAGE,PROCESS_0_ONLY=.FALSE.) ; RETURN
      ENDIF
      IF ((SHAPE_TYPE==HOLE_CYLINDER_TYPE .OR. SHAPE_TYPE==HOLE_CONE_TYPE) .AND. HEIGHT<0._EB) THEN
         WRITE(MESSAGE,'(A,I0,A)')  'ERROR(603): HOLE ',N,' SHAPE requires HEIGHT.'
         CALL SHUTDOWN(MESSAGE,PROCESS_0_ONLY=.FALSE.) ; RETURN
      ENDIF
      IF (SHAPE_TYPE==HOLE_BOX_TYPE .AND. (LENGTH<0._EB .OR. WIDTH<0._EB .OR. HEIGHT<0._EB)) THEN
         WRITE(MESSAGE,'(A,I0,A)')  'ERROR(604): HOLE ',N,' BOX SHAPE requires LENGTH, WIDTH, HEIGHT.'
         CALL SHUTDOWN(MESSAGE,PROCESS_0_ONLY=.FALSE.) ; RETURN
      ENDIF

      ! Compute rotation matrices for shapes
      SELECT CASE(SHAPE_TYPE)
         ! CASE(OBST_SPHERE_TYPE) ! Not needed
         CASE(OBST_CYLINDER_TYPE)
            CALL ROTATION_MATRIX(ROTMAT,ORIENTATION,THETA)
         CASE(OBST_CONE_TYPE)
            CALL ROTATION_MATRIX(ROTMAT,ORIENTATION,THETA)
      ...
      K_MULT_LOOP: DO KK=MR%K_LOWER,MR%K_UPPER
         J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
            I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
               ...
               ! Ignore holes that are outside shape hull

               IF (SHAPE_TYPE>0) THEN
                  AABB = (/X1,X2,Y1,Y2,Z1,Z2/)
                  SELECT CASE (SHAPE_TYPE)
                     CASE (HOLE_SPHERE_TYPE);   IS_INTERSECT = INTERSECT_SPHERE_AABB(XYZ,RADIUS,AABB)
                     CASE (HOLE_CYLINDER_TYPE); IS_INTERSECT = INTERSECT_CYLINDER_AABB(XYZ,HEIGHT,RADIUS,ROTMAT,AABB)
                     CASE (HOLE_CONE_TYPE);     IS_INTERSECT = INTERSECT_CONE_AABB(XYZ,HEIGHT,RADIUS,ROTMAT,AABB)
                     CASE (HOLE_BOX_TYPE);      IS_INTERSECT = INTERSECT_OBB_AABB(XYZ,LENGTH,WIDTH,HEIGHT,ROTMAT,AABB)
                  END SELECT
